class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, name, price, funds = 0, stock = 0):
        self.name = name
        self.price = price
        self.funds = funds 
        self.stock = stock  
    
    def restock(self, amount_to_restock):
        # method to increment the stock attribute of the object
        self.stock += amount_to_restock
        print("'Current {0} stock: {1}'".format(self.name, self.stock))
        

    def add_funds(self, funds_to_add):
        # method to add funds to the funds attribute of the object
        if self.stock == 0:
            print("'Nothing left to vend. Please restock. Here is your $15.'")
        else:
            self.funds += funds_to_add
            print("'Current balance: ${0}'".format(self.funds))
    
    def vend(self):
        #method to vend one item for the given price
        if self.stock == 0:
            ...
            delta = self.price - self.funds
            print(f"'You must add ${delta} more funds.'")
            # print("'You must add ${0} more funds.'".format(self.price))
...
def two_list(vals, amounts):
    ...
    while vals != []:
        vals_end = vals.pop(-1)
        amounts_end = amounts.pop(-1)
        for i in range(amounts_end):
            l = Link(vals_end, l)
    return l
