...
class Ant(Insect):
    ...
    def add_to(self, place):
        #self is the ant being added
        #place.ant is the original ant occupying the space
        if place.ant is None:
            place.ant = self
        else:
            # BEGIN Problem 8
            
            #If the ant originally occupying a place(place.ant) can contain the ant
            #being added(self), then both ants occupy the place and original ant 
            #contains the ant being added.
            if place.ant.can_contain(self):
                place.ant.store_ant(self)
            #If the ant being added(self) can contain the ant originally 
            #in the space(place.ant), then both ants occupy the place and the (container(self)) 
            #ant being added contains the original ant 
            elif self.can_contain(place.ant):
                self.store_ant(place.ant)   
                place.ant = self
            #If neither Ant can contain the other, raise the same AssertionError 
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place.is_hive != True:
            if place.bees != [] and self.min_range <= transitions <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        place = self.place
        bees = list(place.bees) #create copy of bees list

        
        Ant.reduce_health(self,amount) #use Ant.reduce_health to handle logic on the fireant

        #iterate through the bees list to apply reflected damage on each turn and extra damage if fireant removed
        for b in bees:
            if self.health <= 0:
...
# The WallAnt class
class WallAnt(Ant):
    """Wall ant protects our home base."""
    name = 'Wall'
    damage = 0
    food_cost = 4
    implemented = True
...
# The HungryAnt Class
class HungryAnt(Ant):
    """Hungry ant can eat a random bee within it's place
       After eating a bee it must wait 3 turns to eat another bee."""
    ...
    def action(self, gamestate):
        ...
        if self.chew_countdown > 0:
            ...
            rand_bee = random_bee(bees)
            Insect.reduce_health(rand_bee, rand_bee.health)
...
class TankAnt(ContainerAnt):
    """TankAnt provides some heavy duty offense to destroy the Bees."""
    name = 'Tank'
    food_cost = 6
    implemented = True
    damage = 1
...
class ScubaThrower(ThrowerAnt):
    """Ant that can be placed on water."""
    name = 'Scuba'
    food_cost = 6
    implemented = True
    is_waterproof = True
...
# END Problem 12
    ...
    implemented = True   # Change to True to view in the GUI
    # END Problem 12
    queen_count = 0 # first queen has this set to True so it can be initialized

    def __init__(self, health=1):
        super().__init__(health)
        self.queen_count = QueenAnt.queen_count # set equal to False after initialization to prevent futher queens
        # if self.queen_count == 0:
        #     QueenAnt.queen_count += 0
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if cls.queen_count == 0:
            # print("TrueQueen = True")
            cls.queen_count += 1
