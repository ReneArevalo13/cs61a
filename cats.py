...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    true_paragraphs = [
        paragraphs for paragraphs in paragraphs if select(paragraphs) == True]
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    def apply_diff(typed_word, valid_words, diff_function, limit):
        diff = []
        for k in range(len(valid_words)):
            diff.append(diff_function(typed_word, valid_words[k], limit))

        index_min = min(range(len(diff)), key=diff.__getitem__)
        min_value = min(diff)
        same_diff = len(set(diff))

        return index_min, min_value, same_diff

    index_min, min_value, same_diff = apply_diff(
        typed_word, valid_words, diff_function, limit)

    #index_min = apply_diff(typed_word, valid_words, diff_function, limit)

    #for k in range(len(valid_words)):
        #if typed_word == valid_words[k]:
...
def report_progress(typed, prompt, user_id, send):
    ...
    def progress(typed, prompt):
       
        if len(typed) < 1:
            return 0
        if typed[0] == prompt[0]:
            return 1 + progress(typed[1:], prompt[1:])
        else: 
            return 0


    progress_ratio = progress(typed, prompt) / len(prompt)

    report = {
        'id': user_id, 'progress': progress_ratio
    }
...
def time_per_word(times_per_player, words):
    ...
    for j in times_per_player:
        timediff.append([j[i+1] - j[i] for i in range(len(j)-1)])
    return game(words, timediff)
    # END PROBLEM 9
