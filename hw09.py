...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r'\b([IVXLMCDM]+)\b', text)
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'[Cc][Ss][\d]+[\W]*|[Cc][Ss][\s][\d]+[\W]*', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'[0-2][\d]:[0-5][\d][Aa|Pp][Mm]|[\d]:[0-5][\d]|[0-2][\d]:[0-5][\d]', text)
